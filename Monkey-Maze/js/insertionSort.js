async function InsertionSort(delay) {
  ...
  bars[0].style.backgroundColor = "#4B0082 ";
  ...
  for (var i = 1; i < bars.length; i++) {
    ...
    document.getElementsByClassName(
      "range"
    )[0].innerText = `The selected element is [${key}]`;

    //Provide  color to the ith bar
    bars[i].style.backgroundColor = "#8A2BE2";
    ...
    while (j >= 0 && parseInt(bars[j].childNodes[0].innerHTML) > key) {
      ...
      bars[j].style.backgroundColor = "#FF4500";
      ...
      for (var k = i; k >= 0; k--) {
        bars[k].style.backgroundColor = "#922ab4";
      }
    }
  ...
  for (var x = 0; x < 15; x++) {
    bars[x].style.backgroundColor = "#320541";
  }
}
